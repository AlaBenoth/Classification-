...
public class InstanceComparator : IComparer<Instance>
{
	 ...
	 */
	public InstanceComparator(int attributeIndex) => this.attributeIndex = attributeIndex;
	 ...
	 * <returns>
	 *   -1 if the attribute value of the first instance is less than the attribute value of the second instance.
	...
	{
		if (((ContinuousAttribute) x.GetAttribute(attributeIndex)).GetValue() <
			((ContinuousAttribute) y.GetAttribute(attributeIndex)).GetValue())
			return -1;
		if (((ContinuousAttribute) x.GetAttribute(attributeIndex)).GetValue() >
			((ContinuousAttribute) y.GetAttribute(attributeIndex)).GetValue())
			return 1;
