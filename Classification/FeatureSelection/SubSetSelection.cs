...
public abstract class SubSetSelection
{
	 ...
	 */
	public SubSetSelection(FeatureSubSet initialSubSet) => this.initialSubSet = initialSubSet;

	protected abstract List<FeatureSubSet>
		Operator(FeatureSubSet current, int numberOfFeatures);

	/**
	 * <summary>
	 *   The forward method starts with having no feature in the model. In each iteration, it keeps adding the
	 *   features that are not currently listed.
	 * </summary>
	 ...
	 */
	protected void Forward(List<FeatureSubSet> currentSubSetList, FeatureSubSet current,
	...
	/**
	 * <summary>
	 *   The execute method takes an {@link Experiment} and a {@link MultipleRun} as inputs. By selecting a candidateList from
	 *   given
