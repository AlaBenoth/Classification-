...
public class InstanceList
{
	...
	{
		list = new List<Instance.Instance>();
		...
		while (line != null)
		{
			...
			{
				var current = new Instance.Instance(attributeList[^1]);
	 ...
	 */
	public InstanceList(List<Instance.Instance> list) => this.list = list;

	/**
	 * <summary> Adds instance to the instance list.</summary>
	 * <param name="instance">Instance to be added.</param>
	 */
	public void Add(Instance.Instance instance) => list.Add(instance);
	...
	public void AddAll(List<Instance.Instance> instanceList) =>
		list = list.Concat(instanceList).ToList();
	 ...
	 */
	public Instance.Instance Get(int index) => list[index];
	 ...
	 */
	public void Sort(int attributeIndex) => list.Sort(new InstanceComparator(attributeIndex));

	public InstanceList GetSectionOfDataSet(int percentage)
	{
		Shuffle(1);
		var index = list.Count * percentage / 100;
		var data = list.ToArray()[..index];
		return new InstanceList(data.ToList());
	}

	/**
         * <summary> Sorts instance list.</summary>
         */
	public void Sort() => list.Sort(new InstanceClassComparator());
	...
	public void Shuffle(int seed) =>
		KFoldCrossValidation<Instance.Instance>.Shuffle(list, new Random(seed));
	...
	public Bootstrap<Instance.Instance> Bootstrap(int seed) =>
		new Bootstrap<Instance.Instance>(list, seed);

	/**
	 * <summary>
	 *   Extracts the class labels of each instance in the instance list and returns them in an array of {@link
	 *   string}.
	 * </summary>
	...
	public List<string> GetAttributeValueList(int attributeIndex)
	{
		...
		foreach (var instance in list)
			if (!valueList.Contains(((DiscreteAttribute) instance.GetAttribute(attributeIndex)).
				GetValue()))
	...
	public Partition StratifiedPartition(double ratio, Random random)
	{
		...
		for (var i = 0; i < Size(); i++)
		{
			var instance = list[randomArray[i]];
			var classIndex = distribution.GetIndex(instance.GetClassLabel());
			if (counts[classIndex] <
				Size() * ratio * distribution.GetProbability(instance.GetClassLabel()))
	...
	public Partition DivideWithRespectToAttribute(int attributeIndex)
	{
		...
		foreach (var instance in list)
			result.Get(
					valueList.IndexOf(
						((DiscreteAttribute) instance.GetAttribute(attributeIndex)).GetValue())).
				Add(instance);
		return result;
	}
	...
	public Partition DivideWithRespectToIndexedAttribute(int attributeIndex, int attributeValue)
	{
		...
		result.Add(new InstanceList());
		foreach (var instance in list)
			if (((DiscreteIndexedAttribute) instance.GetAttribute(attributeIndex)).GetIndex() ==
				attributeValue)
	...
	public Partition DivideWithRespectToAttribute(int attributeIndex, double splitValue)
	{
		...
		result.Add(new InstanceList());
		foreach (var instance in list)
			if (((ContinuousAttribute) instance.GetAttribute(attributeIndex)).GetValue() <=
				splitValue)
				result.Get(0).Add(instance);
			else
				result.Get(1).Add(instance);
	...
	{
		if (list[0].GetAttribute(index) is DiscreteAttribute)
		{
		...
		if (list[0].GetAttribute(index) is ContinuousAttribute)
		{
			var sum = 0.0;
			foreach (var instance in list)
				sum += ((ContinuousAttribute) instance.GetAttribute(index)).GetValue();
			return new ContinuousAttribute(sum / list.Count);
		}
		return null;
	}
	...
	{
		if (list[0].GetAttribute(index) is DiscreteIndexedAttribute)
		{
			var maxIndexSize =
				((DiscreteIndexedAttribute) list[0].GetAttribute(index)).GetMaxIndex();
		...
		if (list[0].GetAttribute(index) is ContinuousAttribute)
		{
			...
			foreach (var instance in list)
				sum += ((ContinuousAttribute) instance.GetAttribute(index)).GetValue();
			var values = new List<double> {sum / list.Count};
			return values;
		}
		return null;
	}
	...
	private Attribute.Attribute AttributeStandardDeviation(int index)
	{
		if (list[0].GetAttribute(index) is ContinuousAttribute)
		{
			...
			foreach (var instance in list)
				sum += System.Math.Pow(
					((ContinuousAttribute) instance.GetAttribute(index)).GetValue() - average, 2);
			return new ContinuousAttribute(System.Math.Sqrt(sum / (list.Count - 1)));
		}
		return null;
	}
	...
	private List<double> ContinuousAttributeStandardDeviation(int index)
	{
		if (list[0].GetAttribute(index) is DiscreteIndexedAttribute)
		{
			...
			for (var i = 0; i < averages.Count; i++)
				averages[i] = averages[i] / list.Count;
			var values = new List<double>();
			for (var i = 0; i < maxIndexSize; i++)
				values.Add(0.0);
			foreach (var instance in list)
			...
			for (var i = 0; i < values.Count; i++)
				values[i] = System.Math.Sqrt(values[i] / (list.Count - 1));
			return values;
		}
		if (list[0].GetAttribute(index) is ContinuousAttribute)
		{
			...
			foreach (var instance in list)
				sum += System.Math.Pow(
					((ContinuousAttribute) instance.GetAttribute(index)).GetValue() - average, 2);
			var result = new List<double> {System.Math.Sqrt(sum / (list.Count - 1))};
			return result;
		}
		return null;
	}
	 ...
	 * <returns>Distribution of the class labels.</returns>
	 */
	public DiscreteDistribution DiscreteIndexedAttributeClassDistribution(int attributeIndex,
		int attributeValue)
	{
		var distribution = new DiscreteDistribution();
		foreach (var instance in list)
	...
	{
		var distributions = new List<DiscreteDistribution>();
		for (var i = 0; i < list[0].AttributeSize(); i++)
	...
	{
		var result = new Instance.Instance(list[0].GetClassLabel());
		for (var i = 0; i < list[0].AttributeSize(); i++)
	...
	{
		var result = new Matrix(list[0].ContinuousAttributeSize(),
			list[0].ContinuousAttributeSize());
		foreach (var instance in list)
		{
		...
		result.DivideByConstant(list.Count - 1);
		return result;
	}
	 ...
	 */
	public List<Instance.Instance> GetInstances() => list;
}