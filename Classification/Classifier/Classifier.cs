...
public abstract class Classifier
{
	...
	public abstract void Train(InstanceList.InstanceList trainSet,
	...
	public bool DiscreteCheck(Instance.Instance instance)
	{
		for (var i = 0; i < instance.AttributeSize(); i++)
			if (instance.GetAttribute(i) is DiscreteAttribute &&
				!(instance.GetAttribute(i) is DiscreteIndexedAttribute))
				return false;
		return true;
	}
	...
	public virtual Performance.Performance Test(InstanceList.InstanceList testSet, Model.Model trainedModel)
	{
		var classLabels = testSet.GetUnionOfPossibleClassLabels();
		var confusion = new ConfusionMatrix(classLabels);
		for (var i = 0; i < testSet.Size(); i++)
		{
			var instance = testSet.Get(i);
			confusion.Classify(instance.GetClassLabel(), trainedModel.Predict(instance));
		}
		return new DetailedClassificationPerformance(confusion);
	}
	 ...
	 */
	public Performance.Performance SingleRun(Parameter.Parameter parameter,
		InstanceList.InstanceList trainSet, InstanceList.InstanceList testSet)
	{
