...
public class DeepNetwork : Classifier
{
	 ...
	 *   Parameters of the deep network algorithm. crossValidationRatio and seed are used as
	 *   parameters.
	 * </param>
	 */
		...
		Parameter.Parameter parameters)
	{
		var partition = trainSet.StratifiedPartition(
			((DeepNetworkParameter) parameters).GetCrossValidationRatio(),
			new Random(parameters.GetSeed()));
		model = new DeepNetworkModel(partition.Get(1), partition.Get(0),
