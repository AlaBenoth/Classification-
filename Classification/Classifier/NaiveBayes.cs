...
public class NaiveBayes : Classifier
{
	 ...
	 * <param name="classLists">
	 *   Instances are divided into K lists, where each list contains only instances from a
	 *   single class
	 * </param>
	 */
	private void TrainContinuousVersion(DiscreteDistribution priorDistribution,
	 ...
	 * <param name="priorDistribution">Probability distribution of classes P(C_i)</param>
	 * <param name="classLists">
	 *   Instances are divided into K lists, where each list contains only instances from a single
	 *   class
	 * </param>
	 */
	private void TrainDiscreteVersion(DiscreteDistribution priorDistribution,
		Partition classLists)
	{
		var classAttributeDistributions = new Dictionary<string, List<DiscreteDistribution>>();
		for (var i = 0; i < classLists.Size(); i++)
			classAttributeDistributions[
					((InstanceListOfSameClass) classLists.Get(i)).GetClassLabel()] =
