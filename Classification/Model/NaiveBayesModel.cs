...
{
	private readonly Dictionary<string, List<DiscreteDistribution>> classAttributeDistributions;
	private readonly Dictionary<string, Vector> classDeviations;
	private readonly Dictionary<string, Vector> classMeans;
	...
	public NaiveBayesModel(DiscreteDistribution priorDistribution,
		Dictionary<string, Vector> classMeans, Dictionary<string, Vector> classDeviations)
	{
	 ...
	 * <param name="priorDistribution">          {@link DiscreteDistribution} input.</param>
	 * <param name="classAttributeDistributions">
	 *   {@link HashMap} of String and {@link ArrayList} of {@link
	 *   DiscreteDistribution}s.
	 * </param>
	 */
	public NaiveBayesModel(DiscreteDistribution priorDistribution,
	...
	/**
	 * <summary>
	 *   The calculateMetric method takes an {@link Instance} and a String as inputs and it returns the log likelihood
	 *   of
	 * </summary>
	 * these inputs.
	...
	/**
	 * <summary>
	 *   The logLikelihoodContinuous method takes an {@link Instance} and a class label as inputs. First it gets the
	 *   logarithm
	 * </summary>
	...
	private double LogLikelihoodContinuous(string classLabel, Instance.Instance instance)
	{
		...
		for (var i = 0; i < instance.AttributeSize(); i++)
		{
			...
			var mi = classMeans[classLabel].GetValue(i);
			var si = classDeviations[classLabel].GetValue(i);
	...
	/**
	 * <summary>
	 *   The logLikelihoodDiscrete method takes an {@link Instance} and a class label as inputs. First it gets the
	 *   logarithm
	 * </summary>
	...
	{
		var logLikelihood = System.Math.Log(priorDistribution.GetProbability(classLabel));
		var attributeDistributions = classAttributeDistributions[classLabel];
		for (var i = 0; i < instance.AttributeSize(); i++)
		{
			var xi = ((DiscreteAttribute) instance.GetAttribute(i)).GetValue();
			logLikelihood +=
				System.Math.Log(attributeDistributions[i].GetProbabilityLaplaceSmoothing(xi));
		}
		return logLikelihood;
	}
}