...
public class DecisionNode
{
	...
	private readonly DecisionCondition condition;
	...
	/**
	 * <summary>
	 *   The DecisionNode method takes {@link InstanceList} data as input and then it sets the class label parameter by
	 *   finding
	 *   the most occurred class label of given data, it then gets distinct class labels as class labels List. Later, it adds
	 *   ordered
	 *   indices to the indexList and shuffles them randomly. Then, it gets the class distribution of given data and finds the
	 *   best entropy value
	 *   of these class distribution.
	 *   <p />
	 *   If an attribute of given data is {@link DiscreteIndexedAttribute}, it creates a Distribution according to discrete
	 *   indexed attribute class distribution
	 *   and finds the entropy. If it is better than the last best entropy it reassigns the best entropy, best attribute and
	 *   best split value according to
	 *   the newly founded best entropy's index. At the end, it also add new distribution to the class distribution .
	 *   <p />
	 *   If an attribute of given data is {@link DiscreteAttribute}, it directly finds the entropy. If it is better than the
	 *   last best entropy it
	 *   reassigns the best entropy, best attribute and best split value according to the newly founded best entropy's index.
	 *   <p />
	 *   If an attribute of given data is {@link ContinuousAttribute}, it creates two distributions; left and right according
	 *   to class distribution
	 *   and discrete distribution respectively, and finds the entropy. If it is better than the last best entropy it
	 *   reassigns the best entropy,
	 *   best attribute and best split value according to the newly founded best entropy's index. At the end, it also add new
	 *   distribution to
		...
		RandomForestParameter parameter, bool isStump)
	{
		...
		this.data = data;
		classLabel = Classifier.Classifier.GetMaximum(data.GetClassLabels());
		leaf = true;
		...
		for (var j = 0; j < size; j++)
		{
			...
			{
				for (var k = 0;
					k < ((DiscreteIndexedAttribute) data.Get(0).GetAttribute(index)).GetMaxIndex(); k++)
				{
				...
				else
				{
					if (data.Get(0).GetAttribute(index) is ContinuousAttribute)
					{
						...
						for (var k = 0; k < data.Size(); k++)
						{
							...
							{
								previousValue =
									((ContinuousAttribute) instance.GetAttribute(index)).GetValue();
							}
							else
							{
								if (((ContinuousAttribute) instance.GetAttribute(index)).GetValue() !=
									previousValue)
								{
									var splitValue = (previousValue +
										((ContinuousAttribute) instance.GetAttribute(index)).GetValue()) / 2;
									previousValue =
										((ContinuousAttribute) instance.GetAttribute(index)).GetValue();
									entropy =
		...
		if (bestAttribute != -1)
		{
			...
			{
				CreateChildrenForDiscreteIndexed(bestAttribute, (int) bestSplitValue, parameter,
					isStump);
			}
			else
			{
	...
	/**
	 * <summary>
	 *   The entropyForDiscreteAttribute method takes an attributeIndex and creates an List of DiscreteDistribution.
	...
	/**
	 * <summary>
	 *   The createChildrenForDiscreteIndexed method creates an List of DecisionNodes as children and a partition with respect
	 *   to
	 *   indexed attribute.
	 * </summary>
	 * <param name="attributeIndex">Index of the attribute.</param>
	...
	{
		var childrenData =
			data.DivideWithRespectToIndexedAttribute(attributeIndex, attributeValue);
		children = new List<DecisionNode>
		{
			new(childrenData.Get(0),
				new DecisionCondition(attributeIndex,
					new DiscreteIndexedAttribute("", attributeValue,
						((DiscreteIndexedAttribute) data.Get(0).GetAttribute(attributeIndex)).
						GetMaxIndex())), parameter, isStump),
			new(childrenData.Get(1),
				new DecisionCondition(attributeIndex,
	...
	/**
	 * <summary>
	 *   The createChildrenForDiscrete method creates an List of values, a partition with respect to attributes and an List
	 ...
	 * <param name="isStump">       Refers to decision trees with only 1 splitting rule.</param>
	 */
	private void CreateChildrenForDiscrete(int attributeIndex, RandomForestParameter parameter,
		bool isStump)
	{
		...
		for (var i = 0; i < valueList.Count; i++)
			children.Add(new DecisionNode(childrenData.Get(i),
	...
	/**
	 * <summary>
	 *   The createChildrenForContinuous method creates an List of DecisionNodes as children and a partition with respect to
	 ...
	 */
	private void CreateChildrenForContinuous(int attributeIndex, double splitValue,
		RandomForestParameter parameter, bool isStump)
	{
		...
		{
			new(childrenData.Get(0),
				new DecisionCondition(attributeIndex, '<', new ContinuousAttribute(splitValue)),
				parameter, isStump),
			new(childrenData.Get(1),
				new DecisionCondition(attributeIndex, '>', new ContinuousAttribute(splitValue)),
				parameter, isStump)
	...
	/**
	 * <summary>
	 *   The prune method takes a {@link DecisionTree} and an {@link InstanceList} as inputs. It checks the classification
	 *   performance
	...
	/**
	 * <summary>
	 *   The predict method takes an {@link Instance} as input and performs prediction on the DecisionNodes and returns the
	 *   prediction
	 *   for that instance.
	 * </summary>
	...
	public string Predict(Instance.Instance instance)
	{
		if (instance is CompositeInstance compositeInstance)
		{
			...
			var distribution = data.ClassDistribution();
			var predictedClass = distribution.GetMaxItem(possibleClassLabels);
			if (leaf)
				return predictedClass;
			foreach (var node in children)
				if (node.condition.Satisfy(compositeInstance))
				{
