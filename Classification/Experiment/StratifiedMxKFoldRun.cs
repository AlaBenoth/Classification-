...
public class StratifiedMxKFoldRun : MxKFoldRun
{
	/**
	 * <summary>
	 *   Constructor for StratifiedMxKFoldRun class. Basically sets K parameter of the K-fold cross-validation and M
	...
	/**
	 * <summary>
	 *   Execute the Stratified MxK-fold cross-validation with the given classifier on the given data set using the
	 *   given parameters.
	 * </summary>
	...
	public override ExperimentPerformance Execute(Experiment experiment)
	{
		...
		for (var j = 0; j < M; j++)
		{
			var crossValidation = new StratifiedKFoldCrossValidation<Instance.Instance>(
				experiment.GetDataSet().GetClassInstances(), K, experiment.GetParameter().GetSeed());
