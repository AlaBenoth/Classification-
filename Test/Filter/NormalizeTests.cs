...
public class NormalizeTests : ClassifierTests
{
	[Test]
	public void TestLinearPerceptron()
	{
		...
		Assert.AreEqual(2.67, 100 * linearPerceptron.Test(iris.GetInstanceList()).GetErrorRate(),
			0.01);
		normalize = new Normalize(bupa);
		normalize.Convert();
		linearPerceptron.Train(bupa.GetInstanceList(), linearPerceptronParameter);
		Assert.AreEqual(31.59, 100 * linearPerceptron.Test(bupa.GetInstanceList()).GetErrorRate(),
			0.01);
		normalize = new Normalize(dermatology);
		normalize.Convert();
		linearPerceptron.Train(dermatology.GetInstanceList(), linearPerceptronParameter);
		Assert.AreEqual(1.09,
			100 * linearPerceptron.Test(dermatology.GetInstanceList()).GetErrorRate(), 0.01);
	}
	...
	public void TestMultiLayerPerceptron()
	{
		...
		multiLayerPerceptronParameter =
			new MultiLayerPerceptronParameter(1, 0.5, 0.99, 0.2, 100, 30,
				ActivationFunction.Sigmoid);
		normalize = new Normalize(bupa);
		normalize.Convert();
		multiLayerPerceptron.Train(bupa.GetInstanceList(), multiLayerPerceptronParameter);
		Assert.AreEqual(24.06,
			100 * multiLayerPerceptron.Test(bupa.GetInstanceList()).GetErrorRate(), 0.01);
		multiLayerPerceptronParameter =
			new MultiLayerPerceptronParameter(1, 0.1, 0.99, 0.2, 100, 20,
				ActivationFunction.Sigmoid);
		normalize = new Normalize(dermatology);
		normalize.Convert();
	...
	public void TestKMeans()
	{
		...
		Assert.AreEqual(3.55, 100 * kMeans.Test(dermatology.GetInstanceList()).GetErrorRate(),
			0.01);
	}
}