...
public class DiscreteToContinuousTests : ClassifierTests
{
	[Test]
	public void TestLinearPerceptron()
	{
		...
		Assert.AreEqual(8.80, 100 * linearPerceptron.Test(car.GetInstanceList()).GetErrorRate(),
			0.01);
		...
		Assert.AreEqual(1.67,
			100 * linearPerceptron.Test(tictactoe.GetInstanceList()).GetErrorRate(), 0.01);
		...
		Assert.AreEqual(11.45,
			100 * linearPerceptron.Test(nursery.GetInstanceList()).GetErrorRate(), 0.01);
	}
